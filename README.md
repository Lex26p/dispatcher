...
`CORE-006 — Device Manager` находится в разработке. Step 1 подтверждён commit `c79ff405284dbed1d0dc9ac3340f97dbcfa217cd`. Step 2 фиксирует project/resource access semantics без изменения кода: Device может использоваться в нескольких проектах, attached metrics наследуют его project context, standalone metrics имеют собственные связи, а shared metadata защищена от cross-project изменения. После подтверждения Step 2 следующий шаг — Step 3: durable metadata storage.
...
Текущий спринт `CORE-006`: [`docs/development/sprints/CORE-006.md`](docs/development/sprints/CORE-006.md).
