...
[[nodiscard]] bool login_response(
    ...
    return string_field(object.get(), "type", type) && type == "response" &&
           ...
           json_object_object_get_ex(payload, "session", &session) && session != nullptr &&
           json_object_object_get_ex(session, "user", &user) && user != nullptr &&
           string_field(user, "login", login) && login == kLogin;
}

[[nodiscard]] bool current_session_response(
    const std::string& text,
    const std::string_view expected_id) {
    auto object = parse_json(text);
    if (!object) return false;
...
[[nodiscard]] bool access_allowed_response(
    const std::string& text,
    const std::string_view expected_id) {
    auto object = parse_json(text);
    if (!object) return false;
    ...
    json_object* capabilities = nullptr;
    if (!string_field(object.get(), "type", type) || type != "response" ||
        !string_field(object.get(), "id", response_id) || response_id != expected_id ||
        !json_object_object_get_ex(object.get(), "ok", &ok) || ok == nullptr ||
        json_object_get_boolean(ok) == 0 ||
        !json_object_object_get_ex(object.get(), "payload", &payload) || payload == nullptr ||
        !json_object_object_get_ex(payload, "allowed", &allowed) || allowed == nullptr ||
        json_object_get_boolean(allowed) == 0 ||
        !json_object_object_get_ex(payload, "effective_capabilities", &capabilities) ||
        !json_object_is_type(capabilities, json_type_array)) {
        return false;
    }

    const auto count = json_object_array_length(capabilities);
    for (std::size_t index = 0; index < count; ++index) {
        json_object* value = json_object_array_get_idx(capabilities, index);
        if (value != nullptr && json_object_is_type(value, json_type_string) &&
            std::string_view(json_object_get_string(value)) == "admin") {
            return true;
        }
    }
    return false;
...
[[nodiscard]] bool wait_for_provider(Client& client) {
    for (int attempt = 0; attempt < 50; ++attempt) {
        ...
        if (!client.write(request(id, "current-session", "{}"))) return false;
        std::string response;
...
int main(int argc, char* argv[]) {
    if (argc != 3) {
        std::cerr << "Usage: users-access-service-hub-test-client <host> <port>\n";
        return 2;
    }
    ...
    if (!client.write(request(
            "spoof",
            "current-session",
            R"({"user_id":"forged-user"})")) ||
        !client.read(response) ||
        !is_error(response, "spoof", "auth.invalid_session")) {
        return fail("business payload user_id must not authenticate a request");
    }

    const std::string malformed =
        R"({"type":"request","id":"malformed-auth","service":"users-access.v1","operation":"current-session","payload":{},"auth":{"type":"session","token":"bad"}})";
    if (!client.write(malformed) || !client.read(response) ||
        !is_error(response, "malformed-auth", "hub.invalid_request")) {
        return fail("Hub must reject malformed session authentication");
    }
    ...
    if (!client.write(request("current", "current-session", "{}", token)) ||
        !client.read(response) ||
        !current_session_response(response, "current")) {
        return fail("authenticated current-session");
    }

    if (!client.write(request(
            "evaluate",
            "evaluate-access",
            R"({"scope":{"kind":"global"},"capability":"admin"})",
            token)) ||
        !client.read(response) ||
        !access_allowed_response(response, "evaluate")) {
        return fail("authenticated access evaluation");
    }
    ...
    if (!client.write(request("after-logout", "current-session", "{}", token)) ||
        !client.read(response) ||
        !is_error(response, "after-logout", "auth.invalid_session")) {
        return fail("logged-out session should be invalid");
    }

    std::cout << "Users & Access authenticated Service Hub integration passed\n";
    return 0;
}
